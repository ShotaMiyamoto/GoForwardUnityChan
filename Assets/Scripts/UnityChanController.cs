...
public class UnityChanController : MonoBehaviour {
	...
	Animator animator; //アニメーションさせるためのコンポーネントを入れる
	Rigidbody2D rigid2D; //移動用コンポーネントを入れる
	private float groundLevel = -3.0f; //地面の位置
	private float dump = 0.8f; //ジャンプ速度の減衰
	private float jumpVelocity = 20f;
	private float deadLine = -9f; //ゲームオーバーになる位置
	...
	void Update () {
		//走るアニメーションを再生するため、Animatorのパラメータを調整する
